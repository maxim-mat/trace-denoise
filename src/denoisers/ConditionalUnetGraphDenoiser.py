...
class ConditionalUnetGraphDenoiser(nn.Module):
    def __init__(self, in_ch, out_ch, max_input_dim, num_nodes, graph_data,
        ...
        if self.gnn_pooling is None:
            self.ca1 = CrossAttention(channels=128, q_dim=128, kv_dim=64, size=max_input_dim // 2, emb_dim=time_dim)
            self.ca2 = CrossAttention(channels=256, q_dim=256, kv_dim=128, size=max_input_dim // 4, emb_dim=time_dim)
            self.ca3 = CrossAttention(channels=256, q_dim=256, kv_dim=256, size=max_input_dim // 8, emb_dim=time_dim)
            self.ca4 = CrossAttention(channels=128, q_dim=128, kv_dim=256, size=max_input_dim // 4, emb_dim=time_dim)
            self.ca5 = CrossAttention(channels=64, q_dim=64, kv_dim=128, size=max_input_dim // 2, emb_dim=time_dim)
            self.ca6 = CrossAttention(channels=64, q_dim=64, kv_dim=64, size=max_input_dim, emb_dim=time_dim)

            self.ca1_cond = CrossAttention(channels=128, q_dim=128, kv_dim=64, size=max_input_dim // 2, emb_dim=time_dim)
            self.ca2_cond = CrossAttention(channels=256, q_dim=256, kv_dim=128, size=max_input_dim // 4, emb_dim=time_dim)
            self.ca3_cond = CrossAttention(channels=256, q_dim=256, kv_dim=256, size=max_input_dim // 8, emb_dim=time_dim)
            self.ca4_cond = CrossAttention(channels=128, q_dim=128, kv_dim=256, size=max_input_dim // 4, emb_dim=time_dim)
            self.ca5_cond = CrossAttention(channels=64, q_dim=64, kv_dim=128, size=max_input_dim // 2, emb_dim=time_dim)
            self.ca6_cond = CrossAttention(channels=64, q_dim=64, kv_dim=64, size=max_input_dim, emb_dim=time_dim)
