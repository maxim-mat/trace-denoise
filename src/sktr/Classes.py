...
class Transition:
    def __init__(self, name, label, in_arcs=None, out_arcs=None, move_type=None, prob=None, weight=None, properties={},
                 cost_function=None):
        self.name = name
        self.label = label
        self.in_arcs = set() if in_arcs is None else in_arcs
...
class CostFunctionFactory:
    def __init__(self):
        # Dictionary to hold custom cost calculation rules
        self.rules = {}

    def add_rule(self, move_type, rule_func):
        """
        Add a rule for a specific move type.
        :param move_type: The type of move (e.g., 'sync', 'trace', 'model')
        :param rule_func: A function that takes prob as input and returns the cost
        """
        self.rules[move_type] = rule_func

    def create_cost_function(self):
        """
        Returns a cost function that applies the rules defined in the factory.
        """

        def cost_function(prob, move_type=None):
            if move_type is None:
                # Default behavior when only prob is used
                return prob
            elif move_type in self.rules:
                # Apply the specific rule for the given move type
                return self.rules[move_type](prob)
            else:
                # Default cost function behavior
                return 1e-6 if move_type == 'sync' else 1

        return cost_function


# Example usage:
# factory = CostFunctionFactory()

# Define custom rules for different move types
# factory.add_rule('sync', lambda prob: 0.5 * prob)
# factory.add_rule('trace', lambda prob: 1.5 * prob)
# factory.add_rule('model', lambda prob: 2.5 * prob)

# Create a cost function based on the defined rules
# cost_function = factory.create_cost_function()
...
class PetriNet:
    def __init__(self, name='net', places=None, transitions=None, arcs=None, properties={},
    ...
    def __generate_new_trans(self, trans, trace_trans, counter, cost_function):
        ...
        new_sync_transition = Transition(name=name, label=trans.label, move_type='sync', prob=trace_trans.prob,
